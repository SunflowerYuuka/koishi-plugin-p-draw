...
- `p-draw 连续 <角色>：<阶段1> → <阶段2> → ...` 连续图/过程图（同一角色的变化过程）：第一阶段使用 T2I，后续阶段自动以上一阶段图片作为普通 i2i 输入；全阶段共用同一 seed，角色外观尽量一致，默认横图，支持 `|` 分隔与 `--seed` 覆盖。链式阶段的 i2i 去噪强度沿用 `img2imgDenoise`。**仅保留 `连续` 一个别名**。
...
- 连续图用「第一阶段 T2I，后续阶段以上一阶段图片进行 i2i + 全阶段共用 seed」提升一致性。固定角色的身份 tags（角色名 / 种族 / 尖耳朵等）会**始终注入每一阶段**，保证图跟角色相关；阶段描述里明确改变的外观（发色/瞳色/种族变化等）会自动从固定 tags 中剔除，避免"固定银发把黑化阶段拉回去"。后续阶段上传上一张图到 ComfyUI，链式 i2i 的去噪强度使用 `img2imgDenoise`。
